...
class Id_Model extends Model
{
    ...
    select if(random_flag = 1 ,rand(),1) ran,score_flag,user.ad_slot,quiz.id quiz_id,quiz.quiz_type,quiz.status quiz_status,quiz.title quiz_title,quiz.sub_title quiz_sub_title,quiz.image quiz_image,question.id question_id,question.youtube_code question_youtube_code,question.title question_title,question.image question_image,answer.id answer_id,answer.title answer_title 
    ...
    order by 1,question.id,answer.id",array('id'=>$id));
    }  
    
    public function getAnswers($id){
        return $this->db->select("select * from answer where quiz_id = :id",array('id'=>$id));
    }
